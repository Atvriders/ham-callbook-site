  ...
  return (
    <div ref={rootRef} className="relative group/tools">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 rounded-sm px-3 py-2.5 text-[color:var(--color-text-dim)] hover:text-[color:var(--color-accent)] hover:bg-[color:var(--color-surface)] transition-colors"
        aria-haspopup="true"
        aria-expanded={open}
      ...
      <div
        className={`absolute right-0 top-full mt-1 w-48 border border-[color:var(--color-border)] bg-[color:var(--color-bg)] rounded-sm shadow-lg transition-opacity z-50 ${
          open
